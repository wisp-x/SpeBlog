...
function setComment(val) {
	var neb = val == 0 ? 1:0;
	$.post("index.php", {"mod": "admin", "action": "setComment", "neb": neb}, function(result){
		result = strToJson(result);
		if(result.code == 1) {
			alert("更改成功!");
			return history.go(0);
...
function comment_up(id) {
	if(id != "") {
		var name = $.trim($("#comment #comment_name").val());
		var mail = $.trim($("#comment #comment_email").val());
		var url = $.trim($("#comment #comment_url").val());
		var txt = $.trim($("#comment #comment_txt").val());
		if(txt == "") return alert("请输入评论内容");
		if(txt.length > 225) return alert("评论内容超出限制");
		if(name == "") return alert("请输入评论昵称");
		if(name.length > 64) return alert("昵称长度超出");
		if(mail == "") return alert("请输入评论邮箱");
		if(mail.length > 64) return alert("邮箱长度超出");
		if(!checkMail(mail)) return alert("邮箱格式不正确");
		if(url == "") return alert("请输入评论URL");
		if(url.length > 64) return alert("URL长度超出");
		$.post("index.php", {"mod": "index", "action": "comment_up", "article_id": id, "name": name, "mail": mail, "url": url, "txt": txt}, function(result){
			result = strToJson(result);
			if(result.code == 1) {
				return history.go(0);
			}
...
/** ======= Delete Replys ======= **/
function delReplys(id) {
	if(id != "") {
		var del = confirm("确认删除ID为 "+ id +" 的评论?(删除无法恢复!)");
		if(del) {
			$.post("index.php", {"mod": "admin", "action": "delReplys", "id": id}, function(result){
				result = strToJson(result);
				if(result.code == 1) {
					$("#replys-" + id).remove();
					return alert("删除成功");
